import { FiEdit3, FiUser, FiHome, FiBriefcase, FiCreditCard, FiFileText, FiMapPin } from"react-icons/fi";
    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20">
      ...
      <div className="mt-3 space-y-8">
        <ReviewSection title="Personal Info" icon={<FiUser />} onEdit={() => onEdit(0)}>
          <InfoItem label="Selected Hub" value={data.hub?.name || data.hub || 'N/A'} />
        ...
        <ReviewSection title="Business Info" icon={<FiBriefcase />} onEdit={() => onEdit(1)}>
          <InfoItem label="Business Name" value={data.businessName || 'N/A'} />
          <InfoItem label="Business Type" value={data.businessType === "Other" ? data.otherBusiness : (data.businessType || 'N/A')} />
          <InfoItem label="Daily Sales" value={`₦${data.dailySales || '0'}`} />
        ...
        <ReviewSection title="Financial History" icon={<FiCreditCard />} onEdit={() => onEdit(3)}>
          ...
          {data.hasExistingLoan && data.loans && data.loans.map((loan, idx) => (
            <div key={idx} className="mt-4 pt-4 border-t border-gray-100 space-y-3">
              ...
              <InfoItem label="Lender" value={loan.lender || 'N/A'} />
              <InfoItem label="Outstanding" value={`₦${Number(loan.balance || 0).toLocaleString()}`} />
              <InfoItem label="Repayment" value={`₦${Number(loan.repayment || 0).toLocaleString()} / period`} />
            </div>
          ))}
        </ReviewSection>
